...
struct PlotContext
{
    ...
    std::vector<double> displayBinEdges = {};
...
public:
    ...
    // hanyang style hists
    void setHanyangHist(TH1D *hist, int color, int isData, const std::vector<std::string> &binLabels = {}, double scale = 1.0, std::string xTitle = "m_{#mu#mu}", std::string yTitle = "yield", bool isRatio = false);
    ...
    TLegend *setHanyangLegend(int entries, double textSize = 0.04, double scale = 1.0,
                               bool ratioLayout = false, bool ratioTopLayout = false);
    ...
    // latex
    void drawHeader(const std::vector<std::string> &header, double scale, int doRatio);
    ...
    // utilize function
    void drawStackHistWithRatio(const std::map<std::string, TH1D *> &hists, const std::vector<std::string> &stackOrder, const std::map<std::string, double> &stackUp, const std::map<std::string, double> &stackDown, int reOrder, const std::map<std::string, TH1D *> &ratioHists, PlotContext setup, float mcScaling, const std::map<std::string, int> &colorScheme, const std::map<std::string, std::string> &labels, const std::vector<std::string> &headerTexts, const std::vector<std::string> &aboveTexts, const std::vector<std::string> &belowTexts = {}, const std::vector<std::pair<TH1D *, TH1D *>> &shapeUncerts = {}, const std::map<std::string, std::vector<std::pair<TH1D *, TH1D *>>> &nonStackShapeUncerts = {}, bool includeNonStackMCStat = true, const std::vector<std::string> &binLabels = {});
};
