...
def processing(rdf, recordedModules, branchArray, era, ds=""):
    if "ttbar_overlapping_removal_v3.C" not in recordedModules:
        correctionlib.register_pyroot_binding()
        this_dir = os.path.dirname(os.path.abspath(__file__))
        ROOT.gInterpreter.AddIncludePath(this_dir)
        ROOT.gInterpreter.ProcessLine('#include "ttbar_overlapping_removal_v3.C"')
        recordedModules.append("ttbar_overlapping_removal_v3.C")
    ...
    if "GenPart_isMuonForOverlap" not in branchArray:
        rdf = rdf.Define(
            ...
            "BuildMuonForOverlapMaskV3(GenPart_pdgId, GenPart_status, GenPart_statusFlags, GenPart_pt, GenPart_eta)",
        )
    ...
    if "GenPart_bHadOrigin" not in branchArray:
        rdf = rdf.Define(
            "GenPart_bHadOrigin",
            "GetBHadSourceV3(GenPart_pdgId, GenPart_genPartIdxMother)",
        )
    ...
    if "GenJet_minMuonDr" not in branchArray:
        rdf = rdf.Define(
            "GenJet_minMuonDr",
            "GetGenJetMinMuonDrV3(GenJet_eta, GenJet_phi, GenPart_isMuonForOverlap, GenPart_eta, GenPart_phi)",
        )
    ...
    if "GenJet_bHadIdx" not in branchArray:
        rdf = rdf.Define(
            "GenJet_bHadIdx",
            "GetGenJetBHadV3(GenJet_hadronFlavour, GenJet_pt, GenJet_eta, GenJet_phi, GenPart_bHadOrigin, GenPart_eta, GenPart_phi, GenJet_minMuonDr)",
        )
    ...
    if "GenJet_bHadSource" not in branchArray:
        rdf = rdf.Define(
            "GenJet_bHadSource",
            "GetBGenJetsSourceV3(GenJet_bHadIdx, GenPart_bHadOrigin)",
        )
