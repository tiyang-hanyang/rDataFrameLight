...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the ttbb BJA model on a ROOT file and save a structured prediction .npy."
    )
    parser.add_argument("--input", required=True, help="Input ROOT file path.")
    parser.add_argument("--model", required=True, help="Path to the trained ttbb BJA weights (.pt).")
    parser.add_argument(
        "--output",
        default=None,
    ...
    parser.add_argument("--tree-name", default="Events", help="ROOT tree name.")
    parser.add_argument(
        "--device",
        default=None,
...
def extract_bja_features(file_path, tree_name="Events"):
...
def apply_model(model, data_set, batch_size=1024, device="cpu"):
    x_node, x_global = split_features(data_set)
    scores = []
...
def save_scores_with_ids(output_path, ids, scores, prediction):
...
def main():
    ...
    extracted = extract_bja_features(str(input_path), tree_name=args.tree_name)
    ...
    scores, prediction = apply_model(model=model, data_set=features, batch_size=args.batch_size, device=device)

    save_scores_with_ids(str(output_path), ids, scores, prediction)
