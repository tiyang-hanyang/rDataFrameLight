...
std::shared_ptr<const correction::CorrectionSet> RCorr_MC_cset;
...
double get_rndm(double eta, double phi, float nL, int event, int lumi) 
{
    ...
    double mean = RCorr_MC_cset->at("cb_params")->evaluate({abs(eta), nL, 0});
    double sigma = RCorr_MC_cset->at("cb_params")->evaluate({abs(eta), nL, 1});
    double n = RCorr_MC_cset->at("cb_params")->evaluate({abs(eta), nL, 2});
    double alpha = RCorr_MC_cset->at("cb_params")->evaluate({abs(eta), nL, 3});
...
double get_std(double pt, double eta, float nL) {
    ...
    double param_0 = RCorr_MC_cset->at("poly_params")->evaluate({abs(eta), nL, 0});
    double param_1 = RCorr_MC_cset->at("poly_params")->evaluate({abs(eta), nL, 1});
    double param_2 = RCorr_MC_cset->at("poly_params")->evaluate({abs(eta), nL, 2});
...
double get_k(double eta, string var) {
    ...
    // obtain parameters from correctionlib
    double k_data = RCorr_MC_cset->at("k_data")->evaluate({abs(eta), var});
    double k_mc = RCorr_MC_cset->at("k_mc")->evaluate({abs(eta), var});
...
ROOT::VecOps::RVec<float> pt_resol_var(ROOT::VecOps::RVec<float> pt_woresol, ROOT::VecOps::RVec<float> pt_wresol, ROOT::VecOps::RVec<float> eta, string updn)
{
    ...
    for (size_t i = 0; i < pt_woresol.size(); ++i) 
    {
        ...
        float k_unc = RCorr_MC_cset->at("k_mc")->evaluate({abs(eta[i]), "stat"});
...
ROOT::VecOps::RVec<float> pt_scale(bool is_data, ROOT::VecOps::RVec<float> pt, ROOT::VecOps::RVec<float> eta, ROOT::VecOps::RVec<float> phi, ROOT::VecOps::RVec<int> charge) {
    ...
    for (size_t i = 0; i < pt.size(); ++i) {
        ...
        float a = RCorr_MC_cset->at("a_"+dtmc)->evaluate({eta[i], phi[i], "nom"});
        float m = RCorr_MC_cset->at("m_"+dtmc)->evaluate({eta[i], phi[i], "nom"});
...
ROOT::VecOps::RVec<float> pt_scale_var(ROOT::VecOps::RVec<float> pt, ROOT::VecOps::RVec<float> eta, ROOT::VecOps::RVec<float> phi, ROOT::VecOps::RVec<int> charge, string updn) {
    ...
    for (size_t i = 0; i < pt.size(); ++i) {
        ...
        float stat_a = RCorr_MC_cset->at("a_mc")->evaluate({eta[i], phi[i], "stat"});
        float stat_m = RCorr_MC_cset->at("m_mc")->evaluate({eta[i], phi[i], "stat"});
        float stat_rho = RCorr_MC_cset->at("m_mc")->evaluate({eta[i], phi[i], "rho_stat"});
